...
fish_price=read.csv(file = 'C:/Users/Meryem/Desktop/MEF_BIG_DATA_ANALYTICS/503-R/week3/balik_hal_fiyatlari.csv',
...
It can be purchased at an suitable price in the first 4 months x average.
...
fish_prise=fish_price %>%inner_join(elimanted,by="MAL_ADI")
...
``` {r lowest_volatile }
datamin=  fish_price %>%filter(MAL_TURU=='BALIK') %>%
    mutate (ay=lubridate::month(TARIH),daily_change=(AZAMI_UCRET-ASGARI_UCRET),daily_avg=(AZAMI_UCRET+ASGARI_UCRET)/2) %>%
    mutate(avg_change=(daily_change/2)/daily_avg*100) %>% group_by(MAL_ADI) %>%
    summarise(avg_seasonal_change=mean(avg_change),avg_seasonal=mean(daily_avg)) %>%
    arrange(desc(avg_seasonal_change)) %>% tail(5)
...
``` {r kofana }
kofana=fish_price %>% inner_join(datamin,by="MAL_ADI") %>% 
  mutate (ay=lubridate::month(TARIH),daily_change=(AZAMI_UCRET-ASGARI_UCRET),daily_avg=(AZAMI_UCRET+ASGARI_UCRET)/2)
